...
if df is None or df.empty:
    ...
    if datas:
        ...
        def _recuperacao_da_planilha(df, data_sel, periodo_sel):
            sub = df[df["data"].dt.date == data_sel]
            if periodo_sel and periodo_sel != "—":
                sub = sub[sub["periodo"] == periodo_sel]
            m = sub[sub["metrica_norm"] == "recuperação industrial"]
            if m.empty:
                m = sub[sub["metrica_norm"].str.contains("recupera", na=False)]
            if not m.empty:
                return float(m["valor"].mean())
            return None

        val_planilha = _recuperacao_da_planilha(df, data_sel, periodo_sel)
        val_exibido = val_planilha if val_planilha is not None else y0
        sub_origem = "planilha" if val_planilha is not None else "calculado"

        c1, c2, c3 = st.columns([1.4, 1, 1])
        with c1:
            st.markdown(f"<div class='kpi'><div class='lbl'>Recuperação Industrial</div>"
                        f"<div class='val'>{val_exibido:.2f}%</div>"
                        f"<div class='sub'>{data_sel.strftime('%d/%m/%Y')} · {periodo_sel} · {sub_origem}</div></div>",
                        unsafe_allow_html=True)
        with c2:
            top = res[0]
        ...
        if idx_sel <= 0:
            ...
            # ── Impacto real das variações na Recuperação Industrial ─────────────
            st.markdown("---")
            st.markdown("### 🎯 Impacto real das variações na Recuperação Industrial")

            y_ant = recuperacao_industrial(entradas_ant)
            delta_recup_total = y0 - y_ant

            # Derivada parcial dy/dx_i a partir da análise de sensibilidade de hoje
            sens_map = {}
            for r in res:
                row = r["linha"]
                x0 = entradas.get(row, 0.0)
                dx = x0 * choque if x0 != 0 else choque
                sens_map[row] = r["delta_pp"] / dx if dx != 0 else 0.0

            contrib_raw = []
            for row in INPUT_ROWS:
                v_hoje = entradas.get(row, 0.0)
                v_ant_r = entradas_ant.get(row, 0.0)
                delta_x = v_hoje - v_ant_r
                contrib_pp = sens_map.get(row, 0.0) * delta_x
                if abs(contrib_pp) > 1e-8:
                    contrib_raw.append({
                        "linha": row,
                        "indicador": rotulo(row),
                        "valor_anterior": v_ant_r,
                        "valor_atual": v_hoje,
                        "delta_x": delta_x,
                        "contribuicao_pp": contrib_pp,
                    })

            # Deduplica nomes repetidos (mantém maior contribuição absoluta)
            seen_c = {}
            for r in contrib_raw:
                nm = r["indicador"]
                if nm not in seen_c or abs(r["contribuicao_pp"]) > abs(seen_c[nm]["contribuicao_pp"]):
                    seen_c[nm] = r
            contrib = sorted(seen_c.values(), key=lambda r: abs(r["contribuicao_pp"]), reverse=True)

            sinal_total = "+" if delta_recup_total >= 0 else ""
            cor_total = "#48e0b6" if delta_recup_total >= 0 else "#ff7b8a"
            st.markdown(
                f"<p class='legend'>Recuperação <b>{data_ant.strftime('%d/%m/%Y')}</b>: "
                f"<b>{y_ant:.2f}%</b> &nbsp;→&nbsp; "
                f"<b>{data_sel.strftime('%d/%m/%Y')}</b>: <b>{val_exibido:.2f}%</b> &nbsp;|&nbsp; "
                f"Variação total: <span style='color:{cor_total};font-weight:700'>"
                f"{sinal_total}{delta_recup_total:+.3f} p.p.</span><br>"
                f"<small>A contribuição de cada indicador é estimada por: "
                f"∂Recup/∂x · Δx (aproximação linear de 1ª ordem).</small></p>",
                unsafe_allow_html=True)

            if not contrib:
                st.info("Nenhuma variação de indicador com impacto mensurável encontrada.")
            else:
                vmax_c = max(abs(r["contribuicao_pp"]) for r in contrib[:topn]) or 1
                html_c = []
                for i, r in enumerate(contrib[:topn], 1):
                    w = abs(r["contribuicao_pp"]) / vmax_c * 100
                    pos = r["contribuicao_pp"] >= 0
                    grad = ("linear-gradient(90deg,#1f8f7f,#3dd6c4)" if pos
                            else "linear-gradient(90deg,#a83246,#ff7b8a)")
                    tag = ("<span class='tag up'>↑ recuperação</span>" if pos
                           else "<span class='tag down'>↓ recuperação</span>")
                    sinal = "+" if pos else ""
                    cor = "#48e0b6" if pos else "#ff7b8a"
                    html_c.append(
                        f"<div class='rowit'>"
                        f"<div class='rank'>{i}</div>"
                        f"<div class='name' title=\"{r['indicador']}\">{r['indicador']}{tag}</div>"
                        f"<div class='barwrap'><div class='bar' style='width:{w:.1f}%;background:{grad}'></div></div>"
                        f"<div class='pct' style='width:100px;color:{cor}'>"
                        f"{sinal}{r['contribuicao_pp']:+.4f} p.p.</div>"
                        f"</div>")
                st.markdown("".join(html_c), unsafe_allow_html=True)

                tab_c = pd.DataFrame(contrib)[
                    ["indicador", "valor_anterior", "valor_atual", "delta_x", "contribuicao_pp"]]
                tab_c.columns = [
                    "Indicador",
                    f"Valor {data_ant.strftime('%d/%m/%Y')}",
                    f"Valor {data_sel.strftime('%d/%m/%Y')}",
                    "Δ Indicador",
                    "Contribuição Recup. (p.p.)",
                ]
                with st.expander("📋 Ver tabela completa de impactos / exportar"):
                    st.dataframe(tab_c, use_container_width=True, height=420)
                    st.download_button(
                        "⬇️ Baixar impactos (CSV)",
                        tab_c.to_csv(index=False).encode("utf-8"),
                        f"impacto_{data_ant}_{data_sel}.csv",
                        "text/csv",
                    )